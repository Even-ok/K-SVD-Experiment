...
model.add_module('attention4', CBAM(D_out_lam, 16))
'''
注意，测试的模型要在这里修改！
'''
model.load_state_dict(torch.load("D:\\1课程资料\\大二下\\小学期实验\\ksvd\\model.pth", map_location="cpu"))
model.to(device)
...
with open("list_test_PSNR.txt", "wb") as fp:
    with torch.no_grad():
        ...
        for k, (image_true, image_noise) in enumerate(dataloader_test, 0):
            ...
            io.imshow(InNormalization(image_true_t).numpy().astype('uint8'))
            io.imshow(InNormalization(image_noise_0).numpy().astype('uint8'))
            io.imshow(InNormalization(image_restored_t).numpy().astype('uint8'))
