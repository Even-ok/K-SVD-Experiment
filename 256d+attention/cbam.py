...
class SpatialAttention(nn.Module):    #不需要长宽一致！！
    ...
    def forward(self, x):
        ...
        y = torch.cat([avgOut, maxOut], dim=1)  #y[32, 2, 32, 32]   #把两个拼接在一起
        y = self.sigmoid(self.conv(y))  #y[32, 1, 32, 32]  限制在0，1之间
        return x * y.expand_as(x)
...
class CBAM(nn.Module):
    def __init__(self, channel, reduction=16, kernel_size=7):
        ...
        self.SpatialAtt = SpatialAttention(kernel_size)  #只留spatial注意力看看
