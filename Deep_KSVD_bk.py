...
class SubImagesDataset(Dataset):   #用子图来训练
    ...
    def __getitem__(self, idx):
        ...
        sub_image_noise = sub_image +  np.sqrt(sub_image)*self.sigma * noise
...
class FullImagesDataset(Dataset):   #获取完整图像，用来进行测试
    ...
    def _add_noise_to_image(self, np_image, seed):
        ...
        np_im_noise = np_image + np.sqrt(np_image)*self.sigma*noise
        return np_im_noise
...
class DenoisingNet_MLP(torch.nn.Module):
    def __init__(
        ...
        self.Dict = torch.nn.Parameter(Dict_init)      #训练的参数：self.Dict, self.c和w都是可以被训练的权重,MLP的参数
        self.c = torch.nn.Parameter(c_init)
        self.unfold = torch.nn.Unfold(kernel_size=(self.patch_size, self.patch_size))
        #self.fold = torch.nn.Fold(kernel_size=(self.patch_size, self.patch_size))
        ...
        self.linear4 = torch.nn.Linear(H_3, 1, bias=True)
        #self.linear5 = torch.nn.Linear(H_3, 4, bias=True)

        self.w = torch.nn.Parameter(w_init)
        self.attention1 = CBAM(H_1, 16)
        self.attention2 = CBAM(H_2, 16)
        self.attention3 = CBAM(H_3, 16)
        self.attention4 = CBAM(1, 16)
    ...
    def forward(self, x):   #放进来的大小是128*128  1,1,128,128
        ...
        unfold = self.unfold(x)   #用8*8的核将其平展,[1, 64, 14641]  (只有单通道，所以一直向右延展) 64个小长条，做14641次   [1,64,148836]
        #fold_1 = torch.nn.Fold(output_size=(w, h),kernel_size=(self.patch_size, self.patch_size))
        ...
        # lin_1 = self.change_shape(lin)
        # lin = self.attention1(lin_1)
        # lin = self.restore_shape(lin)
        #SE
        lin = self.linear2(lin).clamp(min=0) #[1, 14641, 64]
        # lin_1 = self.change_shape(lin)
        # lin = self.attention2(lin_1)
        # lin = self.restore_shape(lin)
        #SE
        lin = self.linear3(lin).clamp(min=0) #[1, 14641, 32] 
        # lin_1 = self.change_shape(lin)
        # lin = self.attention3(lin_1)
        # lin = self.restore_shape(lin)
        #SE
        lam = self.linear4(lin)   #[1, 14641, 1]  【1，1，121，121】
        lin_1 = self.change_shape(lam,trans_w,trans_h)
        lin = self.attention3(lin_1)
        #lam_1 = self.linear5(lin)  #把lambda搞成一个向量

        # lam_1 = self.change_shape(lam)
        # lam_1 = self.attention4(lam_1)
        # lam_1 = self.restore_shape(lam_1)


        # lam_arr = lam[0].detach().cpu().numpy()
        # lam_arr1 = lam_arr.reshape((121,121))
        # cam = lam_arr1
        # #cam = cam / np.max(cam)  #为什么要除以最大值？  防止后面超过255
        # cam1 = (cam-np.min(cam))/(np.max(cam)-np.min(cam))
        # cam2 = np.uint8(255 * cam1)
        # io.imshow(cam2)
        # io.imsave("C:\\Users\\lyw10\\Desktop\\learning\\大二下\\小学期实验\\Deep-K-SVD\\lam_pic", cam2)
        # cv2.imshow("lam_pic", cam2)
        # cv2.imsave("C:\\Users\\lyw10\\Desktop\\learning\\大二下\\小学期实验\\Deep-K-SVD\\lam_pic", cam2)
        #把lambda关于位置的矩阵弄出来
        # idx=0
        # lam_arr = np.zeros((121,121),dtype=np.float64)
        # for item in lam[0]:
        #     row = idx//121
        #     col=idx%121
        #     lam_arr[row][col]=item.detach().numpy().astype(np.float64)[0]
        #     idx+=1


        #把attention加在lam上，找到相似性和结构性
        l = lam / self.c   #c_init = (linalg.norm(Dict_init.cpu(), ord=2)) ** 2  求特征值，A的转置共轭矩阵与矩阵A的积的最大特征根的平方根值（又给乘方了）
        ...
        z = self.soft_thresh(y, l)  #[1, 14641, 256](软阈值函数)
        for t in range(self.T):   #T=5
            z = self.soft_thresh(torch.matmul(z, S) + (1 / self.c) * y, l)  #不太懂这一步是干嘛
